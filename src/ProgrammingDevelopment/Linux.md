...
Linux 中的信号量是一种睡眠锁”：正确。当进程获取信号量失败时，会进入睡眠状态，所以信号量属于睡眠锁。
...
- 进程地址空间
    - 进程用户空间
        - 进程在运行时使用的虚拟内存空间的用户态部分
        - 用户空间是进程执行时可用的内存区域，其中包含了程序的代码、数据、堆、栈等。用户空间的大小取决于系统的架构和配置，通常是 3GB 到 4GB 之间。
        - 在用户空间中，每个进程都有独立的地址空间，只能访问自己的用户空间，不能直接访问其他进程的用户空间，相互之间不会干扰，从而实现了进程的隔离和保护。
        - 用户空间的主要部分包括
            - 代码段（Text Segment）： 存放程序的机器语言指令，是只读的，用于存放可执行程序的代码。
            - 数据段（Data Segment）： 存放全局变量和静态变量等数据，包括初始化的和未初始化的数据。
            - 堆（Heap）： 动态分配的内存区域，用于存放程序运行时动态分配的数据，由程序员自行管理
            - 栈（Stack）： 存放函数的局部变量、函数参数、返回地址等信息，由系统自动管理
    - 进程内核空间
        - 内核空间包含了操作系统的核心代码和数据结构，只有在内核态才能访问

## 软件
### 命令行配置

【10分钟带你从0配置最强命令行】 https://www.bilibili.com/video/BV1fdTfzeE8X/?share_source=copy_web&vd_source=4da25d719af47084d6e5f1aad46e01ef
...
**返回上一个文件夹**：`cd -`，这个命令相当于执行了`cd "$OLDPWD"`，其中`$OLDPWD`是一个环境变量，存储了上一个工作目录的路径。当您在终端中使用`cd`命令切换目录时，`OLDPWD`会自动更新为当前目录的路径。
...
### `~/.bashrc`
一个配置文件，位于用户的家目录（`~`）下，主要用于配置和定制用户的 **Bash shell** 环境。
Bash 是 Linux 和 macOS 系统中常用的命令行界面（CLI）程序，`~/.bashrc` 文件会在每次启动新的 Bash shell 会话时自动执行。
...
```
**别名（Alias）设置**：你可以在 `.bashrc` 文件中定义命令别名，使得长命令变得更短或更易记。比如：
```
alias ll='ls -la'
