...
**要求**：每个表格中的字段都应该是原子性的（即不可分割的），也就是说，每一列中的数据必须是单一的。
...
假设有一个学生成绩表，包含学生ID、课程、成绩和课程教师信息：
...
在这个表中，`教师` 依赖于 `课程`，而 `课程` 又依赖于 `学生ID`。所以，`教师` 间接地依赖于主键 `学生ID`，违反了 **第三范式**。

解决方法：

将表拆分，确保非主属性不依赖于其他非主属性：
...
这样，`教师` 只依赖于 `课程`，`成绩` 只依赖于 `学生ID` 和 `课程`，符合 **第三范式**。
